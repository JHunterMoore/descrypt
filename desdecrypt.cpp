...
//TODO stop testing for zero block
...
const int key_shift_sizes[] = {0, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
...
unsigned char* keyShift(unsigned char* key, int shiftDist)
{
	...
	while(currentBit < 28)
	{
		if((leftHalf[((currentBit+28-shiftDist)%28)/8] & bitValue[((currentBit+28-shiftDist)%28)%8])!=0)
	...
	while(currentBit < 56)
	{
		if((rightHalf[(((currentBit+28-shiftDist)%28)+4)/8] & bitValue[(((currentBit+28-shiftDist)%28)+4)%8])!=0)
			newKey[currentBit/8] += bitValue[currentBit%8];
		currentBit++;
	}
